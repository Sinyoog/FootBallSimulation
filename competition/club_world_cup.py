...
4) 대륙 쿼터: 유럽13 / 남미5 / 북미2 / 아시아(+오세아니아)6 / 아프리카6 = 32팀.
   (신민용 확정안 — 실제 FIFA 32팀 대륙 배분을 그대로 쓰지 않고, 이 게임의
   챔스 구조에 맞춰 재배분하면서 아프리카 비중을 의도적으로 더 줌.)
   [2026-09 개편, 신민용 확정: "클럽월드컵은 원래 4개의 챔스였는데 5개로
   늘어났으니 유럽쪽에서 좀 더 참가하는 것으로 숫자를 맞추면 될듯"] 챔스가
   4대륙(유럽/북남미/아시아/아프리카)에서 5대륙(유럽/남미/북미/아시아/
   아프리카)으로 늘면서, 옛 "북남미"(8장)를 남미5+북미2=7장으로 나누고
   (남미가 클럽 전력상 우위라는 기존 챔스 설계 취지를 그대로 반영) 줄어든
   1장을 유럽에 얹어(12→13) 합계 32를 유지한다.
...
CWC_QUOTA = {"유럽": 13, "남미": 5, "북미": 2, "아시아": 6, "아프리카": 6}
...
def get_club_world_cup_field(cwc_year: int) -> dict:
    ...
    반환: {"유럽": [...], "남미": [...], "북미": [...], "아시아": [...], "아프리카": [...]}
    각 대륙 리스트 원소는 _select_continent_field 참고.

    [주의] 대륙별 쿼터(13/5/2/6/6)를 다 못 채우는 경우(게임 극초반이라
