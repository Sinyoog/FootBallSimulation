...
# ── 대회 일정 (주차) - 2026-09 상반기로 재환원, 신민용 확정 ─────────────
# [2026-09 재조정 → 재환원, 신민용 확정] 한때 "챔스는 하반기(28주 이후)에
# 시작 — 여러 경기를 진행해서 볼 수 있으니"로 옮겼었는데, 곧바로 신민용이
# "현실 챔스는 가을에 시작해서 이듬해 봄(결승)에 끝나는 25-26 방식인데
# 이 게임은 25 하나로 압축했다 — 그럼 상반기가 맞지 않나?"라고 재검토
# 요청. 결론(신민용 확정): 슈퍼컵 로직(전년도 우승팀, 시즌 초반 개최)은
# 상/하반기 어느 쪽이든 "그 해 안에 챔스가 끝나기만 하면" 동일하게 잘
# 작동하므로 슈퍼컵 때문에 하반기로 갈 필요는 애초에 없었다. 반면 한 해로
# 압축했을 때 "이 해의 챔피언"을 정하는 결승(제일 상징적인 부분)이
# 상반기(봄 느낌)에 오는 쪽이 하반기(그룹스테이지=가을 느낌에 결승까지
# 욱여넣는 쪽)보다 더 자연스럽다 — 그래서 원래 설계(상반기, 8~23주)로
# 되돌린다. europa_engine.py/conference_engine.py는 이 파일의 상수를 그대로
# 참조(EL_START_WEEK=_cl.CL_START_WEEK 등)하므로 따로 안 고쳐도 자동으로
# 같이 되돌아간다. 국내컵(cup_engine.CUP_ROUND_WEEKS_POOL)의 "1부 합류
# 라운드"도 다시 비게 되는 하반기(30주~)로 되돌려야 한다.
#
# 스위스 방식 개편 당시 설계 그대로: 8주(draw)~23주(결승)가 상반기 안에
# 있다 — 리그 스테이지 9~17주(최대 9경기), 18주 플레이오프, 19~23주
# 토너먼트(32강은 남미/북미만). 국내컵은 이 구간(8~23주)을 피해 뒤(30주~)
# 로 밀려 있다.
CL_START_WEEK = 8             # 추첨 (직전 시즌 최종 순위 기준)
CL_LEAGUE_WEEKS = (9, 17)     # 리그 스테이지 최대 구간(9~17주, 실제 사용 주차 수는 대륙별로 다름)
CL_PLAYOFF_WEEK = 18          # 플레이오프 (단판)
CL_ROUND_WEEKS = {
    "R32":  19,  # 남미/북미(32팀)만 여기서 시작. 다른 대륙은 이 라운드 자체가 없음(정상).
    "R16":  20,
    "QF":   21,
    "SF":   22,
    "F":    23,
...
def simulate_my_cl_match(week, p, day=None):
    ...
    # [2026-08 신설, 신민용 리포트: "챔스도 리그처럼 벤치가 있어야 한다"]
    # 리그(game_engine._check_bench)와 동일 로직 재사용, 비교 기준만 이
    # 경기의 팀 OVR(_team_ovr)로 넘긴다. 출전정지면 벤치 확률은 무의미
    # 하므로 건너뛴다(우선순위: 징계 > 벤치).
