...
# 1부 합류 라운드는 챔스 주차와 안 겹치는 구간에 배치하고, 2부 이하는
# 그런 제약이 전혀 없으니 시즌 초반(5주차~)부터 자유롭게 예선을 치르게
# 한다. [2026-09 재조정 → 재환원] 챔스를 잠깐 하반기(28~43주)로 옮겼다가
# (champions_engine.py 상단 주석 참고) 신민용이 "현실 챔스는 가을 시작~
# 이듬해 봄 결승인데 이 게임은 한 해로 압축했으니 결승이 상반기에 오는
# 쪽이 더 자연스럽지 않냐"고 재검토해 챔스를 다시 상반기(8~23주)로
# 되돌렸다. 그에 맞춰 1부 합류 라운드도 "챔스 주차를 피한 구간"이 다시
# 앞(9~21주)에서 뒤(30주~)로 돌아간다 — 아래 CUP_ROUND_WEEKS_POOL 참고.
# 라운드 하나는 일부러 박싱데이 시즌(달력상 12월 하순~1월 초)에 걸리게
# 배치해서 그 시기 일정이 자연스럽게 촘촘해진다.
#
...
# [2026-09 재조정 → 재환원, 신민용 확정] 챔스를 잠깐 하반기로 옮겼다가
# (champions_engine.py 상단 주석 참고) 다시 상반기(8~23주)로 되돌리면서
# 이 파일 맨 위에서부터 지켜온 "1부가 참가하는 라운드는 챔스 시즌과 안
# 겹쳐야 한다" 원칙에 따라 1부 합류 라운드도 다시 뒤(30주~)로 돌아간다.
# 앞쪽 3라운드(5,6,7주 — 하위 티어만 참가, 애초에 챔스와 무관)는 그대로
# 두고, 뒤 7라운드는 2주 간격으로 30주차부터 시작해서(30,32,34,36,38,
# 40,42) 42주차에 끝난다 — 챔스 결승(23주)과 확실히 안 겹치고, 시즌
# 종료(43주, CLUB_SEASON_END_DAY=300) 전에 끝난다.
...
def simulate_my_cup_match(week, p, day=None):
    ...
    # [2026-08 신설, 신민용 리포트: "국내컵도 리그처럼 벤치가 있어야 한다"]
    # 리그와 동일한 _check_bench 재사용, 비교 기준만 이 대회 팀 OVR로.
    # 출전정지면 벤치 확률은 무의미하므로 건너뛴다(우선순위: 징계 > 벤치).
