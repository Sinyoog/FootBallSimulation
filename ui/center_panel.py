...
class CenterPanel(QWidget):
    ...
    def _build(self):
        ...
        self.btn_transfer = QPushButton("✈ 이적 신청");     self.btn_transfer.setObjectName("actBtn")
        ...
        for b in [self.btn_join,self.btn_transfer,self.btn_standing,self.btn_schedule]:
            row1.addWidget(b)
    ...
    def refresh(self):
        ...
        age = p.get("age", 16)
        can_join     = is_pre and age >= MIN_JOIN_AGE and not self._join_used
        can_transfer = is_mid and age >= MIN_JOIN_AGE and not self._transfer_used
    ...
    def _advance(self):
        ...
        # 4주 진행 후 플래그 초기화 (다음 비시즌에 다시 사용 가능)
        self._join_used     = False
        self._transfer_used = False

        # 4주 진행 후: 비시즌 진입 시 알림만 표시 (오퍼는 버튼 클릭 시에만)
    ...
    def _get_match(self, week, p):
        ...
        row = conn.execute(
            ...
            "AND (home_team_id=? OR away_team_id=?) AND home_score=-1",
    ...
    def _do_join(self):
        ...
        if p["age"] < MIN_JOIN_AGE:
            ...
            return
        self._join_used = True   # 이번 달 사용 표시
        self.btn_join.setEnabled(False)
        offers = generate_offers()
        from ui.offer_window import OfferWindow
        self._offer_dlg = OfferWindow(offers, p.get("language","ko"), self, title="🏟 팀 입단")
        self._offer_dlg.finished.connect(lambda: self._on_join_done(self._offer_dlg))
        self._offer_dlg.show()
    ...
    def _do_transfer(self):
        ...
        from constants import MIN_JOIN_AGE
        if p["age"] < MIN_JOIN_AGE:
            show_toast(self, f"⚠  {MIN_JOIN_AGE}살부터 이적 신청이 가능합니다")
            return
        st = get_state()
        week = st["current_week"]
        if not (12 <= week <= 25):
            show_toast(self, "⚠  이적 신청은 12~25주차에만 가능합니다")
            return
        self._transfer_used = True
        self.btn_transfer.setEnabled(False)
        offers = generate_offers()
        from ui.offer_window import OfferWindow
        self._offer_dlg = OfferWindow(offers, p.get("language","ko"), self, title="✈ 이적 신청")
        self._offer_dlg.finished.connect(lambda: self._on_transfer_done(self._offer_dlg))
        self._offer_dlg.show()

    def _on_transfer_done(self, dlg):
        if dlg.chosen:
            from game_engine import update_player, get_player
            p = get_player()
            update_player(popularity=max(0, p.get("popularity", 0) - 5))
            join_team(dlg.chosen["team_id"], dlg.chosen["salary"])
            if self.main_win: self.main_win.refresh_all()
    ...
    def _do_retire(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self._retire_win = RetireWindow(get_player().get("language", "ko"), self)
