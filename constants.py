...
# 자리만 겹치지 않게 분리했다.
AGING_DECLINE = {
    "god":         [(29,31,0.60),(32,34,1.00),(35,37,1.30),(38,40,2.20),(41,45,3.50),
                     (46,50,4.20),(51,55,5.00),(56,999,5.80)],
    ...
    "elite":       [(29,31,2.00),(32,34,2.67),(35,37,3.17),(38,40,3.83),(41,45,3.00),
                     (46,50,3.30),(51,55,3.60),(56,999,4.00)],
    "pro":         [(29,31,3.00),(32,34,3.33),(35,37,3.00),(38,40,3.00),(41,45,2.33),
                     (46,50,2.50),(51,55,2.70),(56,999,3.00)],
    "semipro":     [(29,31,4.50),(32,34,4.33),(35,37,3.00),(38,40,4.17),(41,45,3.33),
                     (46,50,3.50),(51,55,3.70),(56,999,4.00)],
    "amateur":     [(29,31,4.80),(32,34,4.00),(35,37,3.80),(38,40,3.80),(41,45,2.80),
                     (46,50,3.00),(51,55,3.20),(56,999,3.50)],
    "ordinary":    [(29,31,5.00),(32,34,3.67),(35,37,4.17),(38,40,3.33),(41,45,2.33),
                     (46,50,2.50),(51,55,2.70),(56,999,3.00)],
    "untalented":  [(29,31,5.50),(32,34,4.50),(35,37,4.50),(38,40,3.80),(41,45,2.80),
                     (46,50,3.20),(51,55,3.50),(56,999,3.80)],
}
...
#      권장한다.)
MOMENTUM_SCHEDULES = {
    "relegation_recovery":    {3: (1.00, 4.0), 2: (0.90, 2.5), 1: (0.80, 1.5)},
...
CLUB_STRENGTH_RELEGATION_FLOOR = [
    # [2026-08 버그수정, 신민용 리포트: "명문팀 강등 확률을 1% 미만으로
    # 설계했는데 10년 굴리니 절반이 강등당한다" — 근본 원인 확인]
    # 이 리스트엔 tier=2 항목(즉 "1부에서 밀려나는 것 자체"를 막는 항목)이
    # 아예 없었다 — 있던 건 3/4/5부 밑으로 더 떨어지는 걸 막는 항목뿐.
    # PRESTIGE_WEIGHT_BY_LEVEL(prestige_clubs.py)의 "강등권 거의 0%" 목표는
    # new_game 시점 1회성 스쿼드 시딩(team_strength 뽑기)에만 적용되는
    # 값이라, 그 이후 실제 시즌 시뮬레이션에서 1부 밖으로 떨어지는 건 이
    # 리스트가 유일한 방어선이었는데 정작 1부→2부 경계에 방어선 자체가
    # 없었다 — 그래서 명문팀도 그냥 일반 팀과 똑같은 확률로 1부에서
    # 강등당했다(실측: 16시즌 누적 시 3급조차 54.7% 1부 이탈). tier=3/4/5
    # 항목과 같은 패턴으로 tier=2 항목을 추가한다 — club_strength가 실제로
    # 높게 유지되는(=최근 성적이 실제로 좋은) 팀만 보호되므로 "명문이라
    # 무조건 보호"가 아니라 기존 원칙("강해서 보호") 그대로 유지된다.
    # [2026-08 재조정] 1차 값(threshold 7.0, chance 0.65)으로 16시즌 실측
    # 재검증한 결과, level3(레알/바르샤급)조차 "1부 이탈 경험" 비율이
    # 54.7%→57.2%로 거의 그대로였다 — 원인: club_strength 시딩값 자체가
    # 낮다(CLUB_STRENGTH_SEED_BY_PRESTIGE_LEVEL={3:6.0, 2:4.0, 1:2.0}) +
    # 매 시즌 DECAY(0.85) 때문에, 중위권 성적(delta=0)만 한두 시즌 나와도
    # club_strength가 threshold=7.0 밑으로 금방 떨어져 버려 보호 자체가
    # 거의 발동하지 못했다. threshold를 시딩값 근처로 낮추고(강팀이 어느
    # 정도 부진해도 여전히 보호권 안에 머물도록) chance도 올린다.
    (4.5, 1, 0.85),   # 최상위권 강팀 — 1부 밖으로는 웬만하면 안 떨어짐
    (8.0, 3, 0.75),   # 최상위권 강팀 — 3부 밑으로는 웬만하면 안 떨어짐
    (5.0, 4, 0.55),   # 확고한 강팀 — 4부 밑은 드묾
    (2.5, 5, 0.35),   # 어느 정도 강한 팀 — 5부 밑은 더 드묾
]
...
COUNTRY_LEAGUE_GRADE = {
    ...
    "스웨덴": "B", "그리스": "B",
    # [2026-08 재조정, 신민용 확정] OVR(62~68)·연봉(1500~2500만) 모두 B급
    # 다른 나라들(대한민국70~80, 호주69~79 등)보다 뚜렷이 낮아 C로 하향.
    "남아프리카공화국": "C",
    "호주": "B",
...
COUNTRY_LEAGUE_OVR_OVERRIDE = {
    # [2026-08 재조정] 72~76은 C등급 최상위권 수준까지 올라가 있었는데,
    # 카타르는 리그 재정(오일머니)과 실제 리그 축구 수준을 분리해야 하는
    # 대표 사례라 이보다 낮은 게 맞다.
    "카타르": (67, 74),
    "사우디아라비아": (80, 88),
    # [2026-08 재조정] 80~88은 상단이 너무 좁았다 — MLS가 2026 월드컵
    # 기준으로도 상당한 비중을 차지할 만큼 성장했으므로 상단을 소폭 올림.
    "미국": (79, 90),

    # [2026-08 신설, 신민용 확정 — 등급 그룹(A) 공유 표(OVR_RANGES)를 그대로
    # 쓰면 포르투갈/네덜란드/벨기에/아르헨티나가 전부 똑같은 상한(90~100 or
    # 87~99)을 공유해서 "리그 전체가 다들 초엘리트급"처럼 보이는 문제이
    # 있었다(같은 A등급 내 세분화 부재). 등급 문자(A)는 그대로 두고(연봉
    # 배율·명문팀 보너스 등 다른 시스템과 얽혀 있어 안 건드림) 1부 OVR
    # 범위만 국가별로 세분화한다. team_strength 기반 팀별 배치(상위팀은
    # 범위 상단, 하위팀은 하단)는 그대로 유지되므로, 나라 전체 상한을
    # 낮춰도 그 나라 안의 명문팀(아약스/포르투 등)은 여전히 상대적으로
    # 강하게 나온다 — "리그 전체 상향" 대신 "상위팀 프리미엄"으로 표현되는
    # 구조는 이미 있음. 잠정값 — 실제 생성 분포 검증 후 재조정 여지 있음.
    "포르투갈": (84, 95),
    "네덜란드": (82, 94),
    "벨기에": (81, 93),
    # [2026-08 재조정] 78~90은 과조정이었다 — 실제 리그 강도 비교자료상
    # 아르헨티나가 포르투갈(84~95)과 비슷하거나 오히려 약간 위로 평가되는
    # 경우가 있어, 포르투갈보다 확실히 아래로 떨어뜨린 78~90은 너무 낮음.
    "아르헨티나": (81, 93),

    # [2026-08, doc12/13 CSV 기준 잠정 반영 — 이번 세션에서 실측 재검증은
    # 안 거쳤고, 이전 세션 근거(외국인 투자/오일머니로 리그 상위권만
    # 부풀려진 유형)를 그대로 가져온 값. 추후 필요시 개별 재검증 여지 있음.
    "말레이시아": (50, 56),
    "베트남": (52, 58),
    "태국": (54, 60),
    "인도네시아": (50, 56),
...
COUNTRY_SALARY_CAP = {
    ...
    # [2026-08 재조정, 신민용+GPT 검토 확정] 카타르는 사우디와 같은
    # "오일머니 특례"가 맞다고 확정 — 2022월드컵 개최+국부펀드 자금으로
    # QSL에 실제 거액 투자가 이뤄지는 현재진행형 현실이라, cap/mult 비율이
    # A급 정상군보다 높은 것 자체가 의도와 일치한다. 캡은 그대로 유지.
    "카타르":      20_000_000,   # QSL top 200억 (오일머니 특례로 확정)
    ...
    # ── D급 특수 ──────────────────────────────────────────────────────
    # [2026-08 재조정, 신민용+GPT 검토 확정] UAE는 카타르와 같은 오일머니
    # 특례로 확정 — 캡 유지. 중국은 반대로 판단이 갈렸다: "CSL 한때의
    # 초고액 외국인 영입"은 2021년 광저우 에버그란데 파탄 이후 끝난
    # 과거형 현실이라, 위(COUNTRY_LEAGUE_GRADE)에서 이미 B→C로 낮췄다.
    # 캡 절대값(15억)은 공교롭게도 이미 태국(동일 C급, 실측 기반 15억)과
    # 같은 수준이라 그대로 둬도 C급 정상 범주에 들어맞는다 — 특례가
    # 아니라 그냥 C급 상위권 캡으로 재해석.
    "아랍에미리트":  5_000_000,  # UAE리그 top 50억 (오일머니 특례로 확정)
    "중국":         1_500_000,   # 15억 — C급 정상 범주(태국과 동일 수준)
