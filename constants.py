...
LOWER_TIER_SALARY_CAP = {
    # [2026-07 재조정] tier1 곡선을 base_ovr/base_sal 도입으로 다시 설계하면서
    # (더 이상 floor_ovr 밑에서 flat하지 않고 계속 낮아짐) tier1의 "실질
    # 최저값"이 예전보다 많이 내려갔다 — 그 결과 예전 캡(1부 최저보다 낮게
    # 잡아뒀던 값들)이 이제 오히려 1부 중간 OVR대 값보다 높아져 "2부가 1부보다
    # 비싼" 역전이 새로 생겼다. tier1 OVR30~95 전 구간 최소값 기준으로 안전
    # 마진을 두고 다시 낮췄다.
    "잉글랜드":    120_000,   # (기존 1부 최소 14.5억보다 낮게)
    "스페인":       160_000,   # (기존 1부 최소 19.2억보다 낮게)
    "독일":         150_000,   # (기존 1부 최소 18.3억보다 낮게)
    "이탈리아":      45_000,    # (기존 1부 최소 5.8억보다 낮게)
    "프랑스":        28_000,    # (기존 1부 최소 3.7억보다 낮게)
    "사우디아라비아": 35_000,    # (기존 1부 최소 4.6억보다 낮게)
}
