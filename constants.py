...
#   ST elite:  31→84  34→77  37→71  40→62   (공격수는 빨리 쇠퇴)
#   ST normal: 31→79  34→72  37→66  40→60
#   CB elite:  31→85  34→80  37→76  40→70   (수비수는 완만)
#   GK elite:  31→86  34→83  37→80  40→76   (GK는 가장 늦게 늙음)
#   ※ 멘탈은 신체·기술보다 천천히 깎이되(weight 0.45), 더는 '불멸'이 아니다.
...
AGING_DECLINE = {
    ...
    "elite":     [(29,31,1.30),(32,34,1.76),(35,37,1.76),(38,40,2.38),(41,45,3.08)],
...
#   [수정] 멘탈을 0.00(전혀 안 깎임)에서 0.45로. 멘탈은 OVR의 21~33%를
#   차지하는데 전혀 안 깎이면 노년 OVR이 박제돼(34세 86이 80 유지) 비현실적이었다.
#   '멘탈은 다른 스탯보다 덜 깎인다'(신체의 1/3 수준)로 바꿔 노화 곡선을 정상화.
AGING_GROUP_WEIGHT = {
...
WC_QUAL_48 = {
    "유럽": {
        "cutoff_bottom": 22,   # 54 - 22 = 32개국 → 8조×4팀
        "n_groups": 8, "group_size": 4,
        "direct": 16,          # 조 1위+2위 전원 직행
        "po_teams": 0, "po_winners": 0, "wildcard": 0, "quota": 16,
    },
    "아메리카": {
        "cutoff_bottom": 13,   # 45 - 13 = 32개국 → 8조×4팀
        "n_groups": 8, "group_size": 4,
    ...
    "아시아": {
        ...
        "po_teams": 0, "po_winners": 0, "wildcard": 0, "quota": 10,
    },
    "아프리카": {
        "cutoff_bottom": 18,   # 54 - 18 = 36개국 → 9조×4팀
        "n_groups": 9, "group_size": 4,
        "direct": 9,           # 조 1위 9팀 직행
        "po_teams": 0, "po_winners": 0, "wildcard": 0, "quota": 9,
    },
}
...
SPECIAL_SALARY_COUNTRIES = {
    # 사우디아라비아: A등급 리그지만 S급 연봉 (국부펀드, 비과세, 천문학적 계약)
    "사우디아라비아": "S",
    # 카타르: C등급 리그지만 A급 연봉 (왕가 자본, 노후보장형 고액)
    "카타르": "A",
    # 아랍에미리트: D등급 리그지만 B급 연봉 (왕가 자본, 소득세 0%)
    "아랍에미리트": "B",
    # 멕시코: A등급 리그, 연봉도 A급 (기본값과 같으므로 특별 처리 불필요)
    # → 멕시코는 A등급 리그 연봉을 그대로 사용 (남미 최고 수준의 주급 체계)
}

# 기존 LEAGUE_WEALTH_OVERRIDE는 SPECIAL_SALARY_COUNTRIES로 대체됨
# _calc_salary에서 참조하던 LEAGUE_WEALTH_OVERRIDE → 아래 호환 별칭 유지
...
LOWER_LEAGUE_SALARY_OVERRIDE = {
    # ── S급 5부제 나라 ────────────────────────────────────────
    "잉글랜드":   {4: 72000, 5: 12000},  # 4부(EFL 리그 투) 전업프로 월 600~1800만 / 5부(내셔널리그) 월 250~700만
    "스페인":     {4: 4800,  5: 3200}, # 세군다 페데라시온 / 테르세라 세미프로 (월 ~100/80만)
    "프랑스":     {4: 3600,  5: 2800}, # 나시오날2 / 나시오날3 세미프로 (월 ~90/70만)
    "이탈리아":   {4: 2400,  5: 1600}, # 세리에D 세미프로 / 에첼렌차 아마추어 상한 엄격
    "브라질":     {4: 1200,  5: 480},  # 세리에D / 에스토두알 아마추어 (월 ~30/12만)
    "아르헨티나": {4: 800,   5: 480},  # 프리메라C / 토르네오레히오날 아마추어
    "포르투갈":   {4: 2400,  5: 800},  # 캄페오나투 세미프로 / 디스트리탈 아마추어
    "네덜란드":   {4: 2000,  5: 960},  # 데르더 세미프로 / 피어더 아마추어
    "벨기에":     {4: 2400,  5: 1400}, # 디비전2 세미프로 / 디비전3 세미프로
    "크로아티아": {4: 800},            # 트레차 NL 세미프로 (월 ~20만)
    "독일":       {4: 4800,  5: 2400}, # 레기오날리가 세미프로 / 오베르리가 수당 수준

    # ── A급 4부제 나라 ────────────────────────────────────────
    "대한민국":   {4: 7200},  # K4 세미프로 (월 ~150만, 군복무 선수 평균 포함)
    "일본":       {4: 5600},  # JFL 기업구단 포함 (월 ~120만)
    "미국":       {4: 0},     # USL 리그 투 — NCAA 규정상 무급 (대학생 선수)
    "스위스":     {4: 960},   # 1.리가 클래식 아마추어 (월 ~20만)
    "오스트리아": {4: 720},   # 란데스리가 아마추어 (월 ~15만)
    "덴마크":     {4: 1400},  # 3.디비전 세미프로 (월 ~30만)
    "스웨덴":     {4: 1200},  # 디비전2 세미프로 (월 ~25만)
    "멕시코":     {4: 240},   # 리가TDP 유스/아마추어 (월 ~5만)
    "콜롬비아":   {4: 240},   # 프리메라C 아마추어 (월 ~5만)
    "우루과이":   {4: 480},   # 디비시오날D 아마추어 (월 ~10만)
    "이란":       {4: 800},   # 이란3부 세미프로 (월 ~17만)
    "모로코":     {4: 600},   # 아마추어II (월 ~12만)
    "세네갈":     {4: 200},   # 나시오날2 아마추어 (월 ~4만)
}
