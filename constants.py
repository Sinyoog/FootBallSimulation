...
# [2026-09 신설, 신민용+GPT 협업 확정: "강팀 오퍼가 너무 잦다"] SS/S(1·2부)·
# A(1부) 26명 고정 팀들은 예전엔 전부 항상 26명이라 OVR 마진만 맞으면
# 전 세계 강팀 어디서든 오퍽가 뜰 수 있었다. 이제 이 팀들 중 일부만
# 매 시즌 "1자리 빈(25명)" 상태로 만들어서, 그 빈자리가 있는 팀만
# ai_lifecycle._roll_offer_vacancy_teams()가 골라 game_engine.
# generate_offers()의 오퍼 후보에 남긴다 — 26명 꽉 찬 팀은 자동 제외.
# 리그마다 이번 시즌 "빈자리 팀 비율"을 이 범위에서 랜덤으로 하나 뽑는다
# (0%면 그 리그는 이번 시즌 강팀 오퍼가 아예 없음, 35%면 꽤 여러 팀).
OFFER_VACANCY_LEAGUE_PCT_RANGE = (0.0, 0.35)
# 빈자리로 뽑힐 팀을 정할 때, 팀을 "상위권/중위권/하위권" 3그룹으로 나눠
# 그룹별 가중치로 추첨한다(가중치가 클수록 그 시즌 빈자리가 생기기 쉬움) —
# 하위권일수록 자주, 명문/강팀일수록 드물게(완전히 0은 아님) 빈자리가
# 생겨야 "강팀 오퍼는 희귀해지되 아예 사라지진 않는다"는 목표에 맞다.
OFFER_VACANCY_RANK_WEIGHT = {"top": 10, "mid": 30, "bottom": 60}
# 상위권 판정 = 명문팀(data.prestige_clubs.is_prestige) OR 전 시즌 그 리그
# 순위가 상위 OFFER_VACANCY_TOP_PCT 이내. 명문팀은 성적과 무관하게 항상
# 상위권 취급(레알 마드리드가 전 시즌 8위여도 여전히 상위권) — 반대로
# 비명문팀이 전 시즌 돌풍으로 상위 20%에 들면 그 시즌만 상위권 취급.
# 나머지(명문도 아니고 상위 20%도 아닌 팀)는 그다음 OFFER_VACANCY_MID_PCT만큼
# 중위권, 남는 팀은 전부 하위권.
OFFER_VACANCY_TOP_PCT = 0.20
OFFER_VACANCY_MID_PCT = 0.50
...
INTL_TOURNAMENT_KIND_LABELS = {
    ...
    # 우선이라 이 라벨이 보인다.
    "euro_qual": "🎫 유로 예선",
    # [2026-09 신설, 신민용 리포트: "영국령 버진아일랜드는 북미팀인데
    # 2001 유로 예선이라고 뜬다"] cont_qual이 유럽 전용이던 시절에 만든
    # 위 euro_qual 매핑은 "cont_qual + 지역컵 주기 연도"면 무조건 유로
    # 예선으로 단정했는데, 2026-09에 골드컵(북미) 예선이 같은 kind로
    # 추가되면서 북미 대회까지 "유로 예선"으로 잘못 라벨링됐다(대회명
    # 칸에는 "2001 골드컵 예선"으로 제대로 떠서 같은 줄 안에서 이름과
    # 종류가 서로 모순됐다). 골드컵 예선 자체는 이제 폐지됐지만, 이미
    # 그 예선을 치른 기존 세이브의 과거 기록은 그대로 남아있으므로 이
    # 라벨이 필요하다 — world_browser._effective_kind가 대회명으로
    # 지역컵 티어 예선을 가려내 이 라벨을 쓴다.
    "region_qual": "🎫 지역컵 예선",
    # [2026-09 신설, 예선 4대륙 확장] 예전엔 cont_qual이 유럽 전용이라
    # 위 "cont_qual"/"euro_qual" 두 라벨로 충분했지만, 이제 네 대륙이
    # 전부 같은 kind로 예선을 치른다 — 종류 칸이 전부 "대륙컵 예선"으로
    # 뭉뚱그려지면 어느 대륙 예선인지 구분이 안 된다. kind 자체는
    # 'cont_qual' 그대로 두고(대회 판정 로직 불변), world_browser.
    # _effective_kind가 intl_tournaments.continent 컬럼을 보고 아래
    # 라벨로 갈라준다 — 대회명 문자열 추측이 아니라 컬럼 기준이라
    # 대회명이 바뀌어도 라벨이 틀어지지 않는다.
    # (유럽은 주기에 따라 유로 예선 / 유럽 네이션스컵 예선으로 더 갈린다.)
    "cont_qual_eu": "🎫 유럽 네이션스컵 예선",
    "cont_qual_am": "🎫 남북미 대륙컵 예선",
    "cont_qual_as": "🎫 아시안컵 예선",
    "cont_qual_af": "🎫 아프리카 네이션스컵 예선",
...
INTL_TOURNAMENT_KIND_GLYPHS = {
    ...
    "euro":      "⚡",
    "euro_qual": "🎫",
    # [2026-09 신설] 위 LABELS의 대륙별 예선 4종 — 전부 예선이라 🎫.
    "cont_qual_eu": "🎫",
    "cont_qual_am": "🎫",
    "cont_qual_as": "🎫",
    "cont_qual_af": "🎫",
...
PERSONALITY_EFFECTS = {
    ...
    # [2026-09 신설, 신민용 확정] 홈보이: 홈경기 레이팅 보너스 / 원정경기
    # 페널티 — game_engine._player_perf 최종 평점 계산부(승부욕
    # losing_rating과 같은 지점)에서 is_home 여부로 분기 적용.
    "홈보이":   {"home_rating": 0.35, "away_rating": -0.25},
    # 승부사: 중요경기(is_big_match)에서만 슈팅 시도↑ 대신 정확도(gprob)↓
    # 하이리스크·하이리턴 — game_engine._player_perf의 gprob/shots 계산부.
    "승부사":   {"clutch_shot_mult": 1.25, "clutch_goal_mult": 0.85},
    # 은둔형: 인기도 상승 속도 느림(popularity_gain_mult, _calc_pop에서
    # 골/도움 가산분에만 적용) + 이적 요청 시 감독관계 하락폭이 더 작음
    # ("잡음 없이 조용히" — request_transfer는 플레이어 직접 행동이라
    # '확률'을 낮출 대상이 없어, 대신 이적 요청이 조용히·저마찰로
    # 처리되는 쪽으로 구현했다. transfer_rel_penalty_mult).
    "은둔형":   {"popularity_gain_mult": 0.70, "transfer_rel_penalty_mult": 0.5},
}
...
PHYSICAL_TRAIT_EFFECTS = {
    ...
    # [2026-09 신설, 신민용 확정: "올라운더" — ST sub_role에 이미 같은
    # 이름이 있어 "다재다능"으로 명명] 포지션 미스매치 페널티
    # (POSITION_MISMATCH_PENALTY) 완전 면역 — game_engine.calc_positional_ovr
    # 에서 이 플래그가 있으면 미스매치 차감을 0으로 만든다. 효과가 강해
    # 무난함보다도 희귀하게(가중치 최저).
    "다재다능":   {"mismatch_immune": True},
    # 동안/노화저항: 30대 이후(new_age>=29) 연간 OVR 낙폭(annual_drop)을
    # 완화 — game_engine 시즌 종료 나이 처리부.
    "동안":       {"aging_decline_mult": 0.6},
    # 회복력: 부상 복귀 기간(일수) 단축 — game_engine._apply_injury.
    "회복력":     {"injury_recovery_mult": 0.7},
}
PHYSICAL_TRAITS = list(PHYSICAL_TRAIT_EFFECTS.keys())
# 등장 가중치: 무난함이 흔하고, 천재/몬스터는 희귀
# [2026-09 확장] 다재다능/동안/회복력 3종 추가 — 다재다능은 미스매치
# 페널티를 완전히 없애는 강한 효과라 가장 희귀하게(6), 동안/회복력은
# 기존 특징들과 비슷한 빈도(10)로.
PHYSICAL_TRAIT_WEIGHTS = [34, 12, 8, 14, 12, 10, 10, 6, 10, 10]
...
# 월드컵이나 팀에서 포지션 배정될 때 되는거야"] 주발은 단순 표시값이
# 아니라 포지션 배정의 입력값이다. 그래서 전역 단일 분포(현실 인구
# 비율 오른발 75/왼발 20)를 쓰면 안 된다 — 그러면 전 세계 LB의 75%가
# 오른발이 돼서 "자연스러운 LB"가 오히려 희귀해지고, 배정 페널티가
# 상시로 걸린다. 생성 포지션별로 편향을 주어 선수 풀 자체를
# LB→왼발 중심 / RB→오른발 중심으로 형성한다.
#
# 편향을 줘도 반대발은 충분히 생긴다 — 25%면 풀백 규모(전체의 약 20%,
# 실측 26,516+26,570명) 기준으로 반대발 풀백이 1만 명대로 나온다.
#
# 윙어(LW/RW)는 일부러 중앙과 같은 기본 분포를 쓴다. 인버티드 윙어
# (LW+오른발)는 현대 축구에서 표준에 가까운 정상 자원이라, 주발이
# LW/RW를 결정하는 게 아니라 배정 시 sub_role과 함께 평가된다
# (formation_logic의 주발 보정 참고).
FOOT_LEFT  = "왼발"
FOOT_RIGHT = "오른발"
FOOT_BOTH  = "양발"
...
# {슬롯 포지션: 그 슬롯에 자연스러운 발}
FOOT_SIDED_SLOTS = {
    "LB": FOOT_LEFT,  "LWB": FOOT_LEFT,
    "RB": FOOT_RIGHT, "RWB": FOOT_RIGHT,
}
# 윙어 슬롯 — 반대발이어도 페널티가 없고, sub_role이 "인버티드"면
# 오히려 소폭 보너스를 받는 자리.
FOOT_INVERTED_SLOTS = {
    "LW": FOOT_LEFT, "LM": FOOT_LEFT,
    "RW": FOOT_RIGHT, "RM": FOOT_RIGHT,
}

# [2026-09 신설, D단계 실측 전 잠정값 — 아직 확정치가 아니다] 주발
# 미스매치 페널티와 인버티드 보너스. 두 값 모두 0이면 주발 보정이
# 완전히 무동작이 되도록 formation_logic이 조기 반환하므로, 같은 월드
# A/B의 OFF 기준선을 이 상수만으로 만들 수 있다.
#
# 이 값이 곧 "언제 반대발 대신 실력을 택하는가"의 경계다 —
# 유효 적합도 = OVR × (1 - 포지션 미스매치) × (1 - 주발 미스매치)
# 이므로, 왼쪽 풀백 슬롯에서 OVR 90 오른발과 OVR 60 왼발이 뒤집히는
# 지점은 90 × (1-P) = 60, 즉 P ≈ 0.333이다. 너무 작으면 주발이 장식이
# 되고, 너무 크면 OVR 높은 반대발 선수를 부당하게 밀어낸다 —
# 그래서 이론값으로 박지 않고 D단계에서 같은 월드 A/B로 골랐다.
#
# [D단계 실측 확정, 7시즌 월드 11,399팀 · 신민용 확정] 0.05~0.35를
# 같은 월드에서 sweep한 결과 0.15가 교환비가 꺾이는 무릎이었다:
#   P      반대발   선발OVRΔ  포지션rankΔ  한계효율(반대발%p/OVR)
#   OFF    31.3%     —          —
#   0.05   15.7%   -0.014    -0.0548        1107
#   0.10   13.0%   -0.027    -0.0520         215
#   0.15   10.9%   -0.044    -0.0516         124   <- 채택
#   0.25    8.7%   -0.074    -0.0512          73
#   0.35    7.8%   -0.090    -0.0511          56
# 0.10->0.15는 반대발 2.1%p를 OVR 0.017에 사지만, 0.15->0.25는 2.2%p에
# 0.030을 쓴다. 안전성도 확인됐다 — 교체 시 희생 OVR 중앙값 3,
# 30 이상 희생 0건(P=0.50에서도 0건), 벤치 평균 OVR은 오히려 +0.040.
# 포지션 rank가 같이 개선되는 건 벤치 교환이 "LB 슬롯에 CB가 서 있고
# 진짜 LB가 벤치에 앉아 있던" 배정까지 같이 고치기 때문이다.
#
# [축 분리] 포지션 미스매치(POSITION_MISMATCH_PENALTY)와 합치지 않는다.
# "LB 슬롯에 RB 선수"(포지션 미스매치)와 "LB 슬롯에 오른발 LB"(주발
# 미스매치)는 서로 다른 현상이고, 신체특징 '다재다능'은 앞쪽만 면역이다
# (뒤쪽의 면역은 '양발'이 맡는다 — 축을 겹치지 않게 유지한다).
FOOT_MISMATCH_PENALTY = 0.15
FOOT_INVERTED_BONUS = 0.03
# [2026-09 확정, D단계 실측] 선발 11명과 벤치 사이의 교환까지 허용한다.
# 꺼져 있으면 주발 보정은 이미 뽑힌 11명 안에서의 자리 교환만 하는데,
# 그때는 손해 없는 교환만 일어나 OVR/포지션 적합도가 전혀 안 바뀌는
# 대신 반대발도 19.2%까지밖에 안 내려간다(실측). 켜야 "OVR 90 오른발
# LB를 빼고 OVR 60 왼발 LB를 넣을 것인가" 같은 진짜 트레이드오프가
# 생기고, 그때 비로소 FOOT_MISMATCH_PENALTY 값이 결과를 가른다 —
# 경계는 (선발 OVR) x (1 - P) = (벤치 OVR)이다.
FOOT_BENCH_SWAP = True

# [2026-09 신설, 신민용 리포트: "AI 이적시장이 포지션을 안 보고 사고팔아
# 팀 편중이 심하다"] formation_logic.position_sell_weight / position_demand_
# weight를 ai_lifecycle._do_one_transfer_cached에 연결할 때 쓰는 강도
# 계수다. 두 함수가 돌려주는 원래 배율(raw)을 그대로 쓰지 않고
#     적용배율 = 1.0 + STRENGTH x (raw - 1.0)
# 로 희석해서 쓴다 — 0.0이면 기능 꺼짐(기존과 100% 동일), 1.0이면
# 두 함수 정의 그대로의 전력. "처음부터 강하게 적용하지 말 것"이라는
# 확정 지침에 맞춰 중간값에서 출발한다. 두 함수 자체는 안 건드린다.
#   sell  : mover(누가 팔리는가) 가중치 — 과잉 포지션은 더 잘 팔리고
#           목표에 맞거나 부족한 포지션은 덜 팔린다.
#   demand: 목적지(어느 팀이 사는가) 가중치 — 그 포지션이 부족한 팀이
#           더 적극적으로 사러 간다.
POSITION_SELL_STRENGTH = 1.0
POSITION_DEMAND_STRENGTH = 1.0
...
REGION_TARGET_SIZE = {
    ...
    "동아프리카": 8, "남부아프리카": 12,
    "남미": 10,
    # [2026-09] 골드컵 — 예선 폐지 이후 이 값이 곧 본선 참가국 수다
    # (북미 33개국 중 그 시점 국가 OVR 상위 24개국, EURO_QUAL 주석 참고).
    "북미": 24,
    "오세아니아": 8,
}
...
EURO_QUAL = {
    ...
    # [2026-09 폐지, 신민용 확정: "골드컵 예선 이거 없애줘 — 다 올라가는데
    # 할 이유가 없잖아"] 여기 있던 "북미"(골드컵) 예선 설정을 제거했다.
    # 폐지 이유를 기록으로 남긴다: 북미 33개국에서 하위 9개국을 컷오프한
    # 24개국을 12조×2팀으로 나눴는데, 조 1위 12팀(direct)과 조 2위 12팀
    # (wildcard)이 전부 통과라 진출국 24개국이 예선 참가국 24개국과 항상
    # 정확히 같았다 — 탈락이 구조적으로 불가능한 예선이었다. 골드컵은 이제
    # 다른 모든 지역컵과 동일하게 44주차(INTL_CALLUP_WEEK) 본선 생성 시점에
    # intl_engine._qualify_region("북미")의 OVR 상위컷(REGION_TARGET_SIZE
    # ["북미"]=24)으로 참가국을 정한다.

    # [2026-09 확장, 신민용 확정: "이제 대륙 네이션스컵들 다 예선전 만들어줘"]
    # 아래 3개 대륙은 예전엔 예선 없이 등급+난수로 본선 24개국을 바로
    # 뽑았다(_qualify_continental의 폴백 경로) — 국가 grade가 고정값이라
    # 선수를 아무리 키워도 본선 진출 확률이 전혀 안 올랐다. 이제 유럽과
    # 똑같이 "OVR 하위컷 → 조별리그(홈·원정 2판씩) → 조 1·2위 진출"을 탄다.
    #
    # 조 수는 24장을 정확히 채우도록 대륙마다 다르다(n_groups x 2 = 24):
    #   유럽/아프리카 54개국 → 6컷 → 48 → 12조x4 → 1·2위 24
    #   아시아        58개국 → 10컷 → 48 → 12조x4 → 1·2위 24
    #   아메리카      45개국 → 1컷  → 44 → 11조x4 → 1·2위 22 + 조3위 PO 2
    # 아메리카만 11조라 1·2위로 22장밖에 안 나온다 — 모자란 2장은 조 3위
    # 11팀 중 상위 4팀의 단판 토너먼트로 채운다(신민용 확정: "아메리카는
    # 조 3위 중 가장 점수 높은 애들끼리 단판 토너먼트를 통해 가면 된다").
    # po_pool_rank=3이 그 "3위 풀"을 가리킨다(intl_engine._finalize_qual).
    #
    # [주의] 이 표의 키는 '대륙'(대륙컵 계층, 4키)이다 — 월드컵 예선이
    # 쓰는 '연맹'(WC_QUAL_32/48, 남미·북미를 나눈 5키)과는 다른 계층이라
    # 서로 섞어 쓰면 안 된다(intl_engine._cont_key / _conf_key 주석 참고).
    "아메리카": {
        "cutoff_bottom": 1,     # 45 - 1 = 44개국 → 11조×4팀
        "n_groups": 11, "group_size": 4, "legs": 2,
        "direct": 11,           # 조 1위 11팀 전원 직행
        "wildcard": 11,         # 조 2위 11팀 전원 추가 (11+11=22)
        "po_teams": 4, "po_winners": 2, "po_pool_rank": 3,
        "quota": 24,            # 22 + PO 승자 2 = 24
    },
    "아시아": {
        "cutoff_bottom": 10,    # 58 - 10 = 48개국 → 12조×4팀
        "n_groups": 12, "group_size": 4, "legs": 2,
        "direct": 12, "wildcard": 12,
        "po_teams": 0, "po_winners": 0,
        "quota": 24,
    },
    "아프리카": {
        "cutoff_bottom": 6,     # 54 - 6 = 48개국 → 12조×4팀 (유럽과 동일)
        "n_groups": 12, "group_size": 4, "legs": 2,
        "direct": 12, "wildcard": 12,
        "po_teams": 0, "po_winners": 0,
        "quota": 24,
...
# [2026-09 개편] "아메리카"(13장) → 남미 + 북미로 분리(합계 48 불변).
# [2026-09 버그수정, 신민용 리포트: "2022년부터 월드컵이 안 열린다"]
# 위 개편 때 이 표만 남미 7 / 북미 6으로 바뀌고, 실제 예선 구조를
# 정의하는 아래 WC_QUAL_48은 그대로 남미 6 / 북미 7이었다(각 연맹의
# "quota" 필드 및 조 편성이 배출하는 팀 수 모두 6/7). 합계는 양쪽 다
# 48이라 총합 검증에는 안 걸렸지만, _qualify_world()의 연맹별 검증
# (actual < quota → RuntimeError)에서 남미가 6 < 7로 매번 걸려
# 월드컵 본선 생성 자체가 통째로 실패했다 — 32팀 체제(~2018)는 다섯
# 연맹이 전부 정확히 맞아서 멀쩡했고, 48팀으로 전환되는
# WC_EXPAND_YEAR(2022)부터 모든 월드컵이 사라진 원인. 예선 구조 쪽을
# 그대로 두고 이 표를 6/7로 맞춘다(현실 CONMEBOL 6장과도 일치).
WC_QUOTA_BIG = {"유럽": 16, "남미": 6, "북미": 7, "아시아": 10, "아프리카": 9}
...
# 한다"] 위 PRESTIGE_SCOUT_*(_prestige_scouting)는 "지금 당장의 실력
# 업그레이드"만 본다 — 후보를 현재 OVR 상위 퍼센타일에서 찾으므로, 잠재력은
# 높지만 소속팀 team_cap에 막혀 현재 OVR이 낮은 선수는 애초에 후보 풀에
# 들어오지도 못한다. ai_lifecycle._prestige_potential_scouting은 현재 OVR이
# 아니라 "potential_ovr − 소속팀 team_cap" 격차만 보는 별도 통로 —
# 데려오는 시점엔 당장 도움이 안 되는(심지어 벤치보다 약한) 선수일 수
# 있다는 게 핵심(현실의 "잠재력 보고 데려오는 유스 영입"에 대응).
#
# [2026-09 수정, 신민용 지적: "한계재능(potential_ovr) 90은 리버풀한테
# 벤치급이다 — 왜 그런 선수를 발굴하러 가나" + "6시즌 검증에서 22세
# OVR60짜리가 곧장 리버풀로 간 건 비현실적"] 두 가지를 고친다.
# (1) 발굴 문턱을 전 세계 공통 90 하나로 두지 않고 목적지 명문 레벨별로
#     차등화 — 3급(레알/리버풀/PSG급)은 진짜 세계적 재목(96+)만, 2급은
#     93+, 1급(그냥 "국내 명문" 수준)은 기존 90 그대로. 레벨이 낮을수록
#     상대적으로 덜 대단한 원석도 노려볼 만하다는 뜻(PRESTIGE_SCOUT_BAND가
#     이미 "3급은 최상위 0.5%만, 1급은 상위 10%까지"로 등급별 눈높이를
#     나누는 것과 같은 원리).
# (2) potential_ovr만 보고 현재 실력을 전혀 안 보면 "17세 잠재력만 있고
#     현재 OVR은 바닥"인 선수까지 명문팀이 곧장 데려가는 비현실적인
#     그림이 나온다 — 나이별로 "그래도 이 정도는 이미 보여준 상태"라는
#     현재 OVR 하한을 같이 요구한다(나이가 어릴수록 낮게, 다 클수록
#     높게 — 22세인데 아직 OVR 60이면 잠재력이 아무리 높아도 발굴 서사
#     자체가 성립하지 않는다는 지적 반영).
POTENTIAL_SCOUT_MIN_POTENTIAL_BY_LEVEL = {3: 96, 2: 93, 1: 90}
# [2026-09 수정, 신민용 지적: "22세인데 OVR60이 리버풀 스카우팅 대상이 되는
# 건 말이 안 된다"] 처음엔 나이별 절대 OVR 숫자(17세60/18세65.../22세78)로
# 뒀는데, 헤드리스로 돌려보니 후보가 403명 → 2명으로 폭락했다 — 원인은
# team_cap이 낮은 나라(한국 75 등)는 그 나라 안에서 아무리 잘 커도 절대
# 숫자 자체(78)를 넘을 수 없어서, 정작 이 기능을 만든 이유였던 "한국 등
# 하위리그 원석"이 전부 걸러져버렸기 때문이다(이 gap 조건을 만족하는
# 선수는 정의상 자기 team_cap보다 한참 위의 잠재력을 가졌으니, 절대
# 숫자가 아니라 "자기 team_cap 대비 상대적으로 어디까지 왔는가"로 봐야
# 한다). 그래서 절대 OVR이 아니라 "소속팀 team_cap 대비 비율"로 바꾼다 —
# 나이가 많을수록 자기 환경의 상한에 더 가까이 와 있어야 발굴 후보로
# 본다(22세인데 team_cap의 88%도 못 왔으면 "그 좁은 리그에서도 아직
# 안 통하는" 선수라 발굴 서사가 안 맞는다는 뜻).
POTENTIAL_SCOUT_MIN_OVR_FRAC_BY_AGE = {
    17: 0.55, 18: 0.62, 19: 0.70, 20: 0.77, 21: 0.83, 22: 0.88, 23: 0.90,
}
# [2026-09 추가, 신민용 지적: "한계수치(잠재력) 90인 선수를 리버풀이 왜
# 데려가나 — 벤치급인데" 확인 중 재발견] 위 team_cap 대비 상대 하한만으론
# "자기 나라에선 최상급이지만 절대치로는 OVR 20~30대"인 선수까지 레벨3
# (리버풀·바이에른급) 목적지로 그대로 통과했다(3부 국가리그처럼 team_cap
# 자체가 28인 나라면 그 90%인 25만 넘어도 통과돼버림) — 잠재력이 아무리
# 좋아도 이 정도 절대 수준은 명문팀이 직접 데려갈 원석이 아니라 그 아래
# 단계(자국 상위팀·해외 중소리그)가 먼저 채갈 몫이다. 목적지 레벨별
# 절대 OVR 하한을 추가로 요구한다(레벨이 높을수록 기준도 높다).
POTENTIAL_SCOUT_MIN_ABS_OVR_BY_LEVEL = {3: 60, 2: 50, 1: 42}
POTENTIAL_SCOUT_MIN_GAP = 15         # potential_ovr - 소속팀 team_cap 최소 격차
POTENTIAL_SCOUT_MAX_AGE = 23         # 이 나이 이하만 대상(다 큰 선수는 발굴 서사에 안 맞음)
# 명문 레벨별 "이번 시즌에 이 팀이 실제로 발굴을 시도할 확률" — 시즌마다
# 전 세계 명문팀이 다 같이 도는 게 아니라 가끔 한 팀만 걸리는 정도의
# 희귀한 이벤트로 남긴다(3급일수록 스카우트 네트워크가 넓다고 보고 조금
# 더 자주 시도).
POTENTIAL_SCOUT_PROB_BY_LEVEL = {3: 0.35, 2: 0.15, 1: 0.06}
...
# [2026-09 신설, 신민용 리포트: "RW 90/83/82가 있어도 지금 구조에서는
# RW가 최대 1명만 뽑힌다"] database.get_country_squad_players가 AI들끼리
# 26인 국가대표를 뽑을 때 쓰던 "포지션당 고정 슬롯"(_INTL_MATCHDAY_
# FULL_POS, intl_engine.py — GK×3/CB×4/LB×2/RB×2/CM×4/CDM×1/CAM×2/
# LW×1/RW×1/ST×6) 구조는 위 INTL_SQUAD_QUOTA(내 선수 전용, 4개 대분류)
# 와는 별개의 문제 — 슬롯 자체가 포지션 "문자열"마다 딱 1~6자리로 고정돼
# 있어서, 그 포지션에 아무리 강한 후보가 많아도 슬롯 수 이상은 절대
# 못 들어간다(윙어처럼 슬롯이 1개뿐인 포지션이 특히 심함). 그래서 이
# 26인 선발을 "포지션 그룹별 목표 인원"으로 다시 설계한다 — 그룹 안의
# 후보끼리 자유 경쟁시켜 목표 인원까지 채우므로, RW가 여러 명 강하면
# 그만큼(그룹 목표 인원 한도 내에서) 다 들어올 수 있다.
INTL_POSITION_GROUPS = {
    "GK": ["GK"],
    "CB": ["CB"],
    "FB": ["LB", "RB"],
    "CM": ["CDM", "CM"],
...
# [2026-09 확정, 신민용 승인] 그룹별 목표 인원 — 합계 26(기존 26인 원칙
# 그대로 유지). 이 표만 조정하면 스쿼드 구성 비율이 바뀐다.
INTL_SQUAD_GROUP_QUOTA = {
    "GK": 3, "CB": 5, "FB": 4, "CM": 6, "AM": 2, "WG": 4, "ST": 2,
}
INTL_SQUAD_TOTAL = sum(INTL_SQUAD_GROUP_QUOTA.values())
INTL_POSITION_TO_GROUP = {pos: grp for grp, members in INTL_POSITION_GROUPS.items() for pos in members}

# [2026-09 신설, 신민용+advisor 확정 — "②포지션 적합도, 방향성이 중요
# 하다"] 그룹 "핵심" 포지션이 아닌 인접 포지션도 그 그룹의 26인 경쟁에
# 후보로 넣되, 최종 선발점수에 곱하는 적합도 배율로 세기를 조절한다.
# 핵심(정확히 그 그룹 소속)은 코드에서 항상 1.00으로 처리하므로 여기엔
# 없다. 방향이 대칭이 아니다 — 예: 윙어가 스트라이커로 전환되는 쪽
# (WG→ST 0.55)이 스트라이커가 윙으로 가는 쪽(ST→WG 0.45)보다 살짝
# 쉽다(드리블/크로스 능력이 있는 윙어가 최전방으로 가는 게, 정통
# 타겟맨이 측면으로 가는 것보다 현실적으로 더 자연스럽다는 판단).
# 기존 POSITION_COMPAT(클럽 포메이션 배치가 쓰는 표)이 "그 관계가
# 존재하는지"는 이미 담고 있지만, 정도(숫자)까지는 없어서 —
# advisor 확정 예시(WG 그룹 기준 핵심 1.00 / CF류 0.70~0.80 / ST류
# 0.40~0.55, 이 게임엔 CF 태그가 없어 가장 가까운 실존 포지션인 CAM로
# 대응) 그대로 다른 그룹에도 같은 원리(그룹에 인접한 포지션 0.55~0.75,
# 더 먼 포지션 0.30~0.45)를 확장 적용한다. {그룹: {포지션: 적합도}}.
INTL_GROUP_FIT = {
    "WG": {"ST": 0.45, "CAM": 0.65},
    "ST": {"LW": 0.55, "RW": 0.55, "CAM": 0.60},
    "AM": {"ST": 0.45, "LW": 0.60, "RW": 0.60, "CM": 0.70, "CDM": 0.30},
    "CM": {"CAM": 0.75},
    "FB": {"CB": 0.55},
    "CB": {"LB": 0.55, "RB": 0.55},
}

# [2026-09 신설, 신민용 확정: "한쪽에 좋은 선수가 몰려서 반대쪽이 0명"]
# 좌우가 한 그룹으로 묶인 그룹의 (왼쪽, 오른쪽) 포지션. 실측에서
# 12개국 중 2개국(이탈리아 LW4/RW0, 멕시코 LW4/RW0)이 윙어 한쪽 0명으로
# 뽑혔고, 풀백도 3:1 편중이 흔했다 — INTL_POSITION_GROUPS가 FB=[LB,RB],
# WG=[LW,RW]로 묶고 점수순으로만 뽑아 좌우 제약이 전혀 없었기 때문.
#
# 쿼터를 2/2로 쪼개지는 않는다 — 그러면 "오른쪽에 월드클래스가 3명
# 있어도 2명만"이라는 경직성이 생긴다. 각 방향 최소 1명만 먼저 확보하고
# 나머지는 기존 선발 점수순으로 자유롭게 채운다. 그 방향 후보가 아예
# 없는 나라에서는 강제하지 않는다(없는 RB를 억지로 만들지 않는다).
INTL_GROUP_SIDES = {
    "FB": ("LB", "RB"),
    "WG": ("LW", "RW"),
}
...
AWARD_POS_GOAL = {"ST":18,"CF":14,"LW":9,"RW":9,"CAM":7,"CM":4,"GK":0.48}
# 포지션별 시즌 기대 도움 베이스 (마찬가지로 OVR 무관 고정 기준치)
AWARD_POS_ASSIST = {"CAM":11,"CM":8,"LW":8,"RW":8,"CF":6,"ST":5,"GK":0.48}
# [2026-09 신설, 신민용 확정: "GK도 득점은 가능하지, 무조건 하는게 문제인거지 /
# 세트피스 담당 여부 같은 별도 능력치를 추가할 필요는 없다"] 이 표에 GK 항목이
# 없어서 game_engine._estimate_ai_season이 .get(pos, 1) 기본값 1로 떨어뜨렸고,
# 그 결과 GK가 CB와 같은 기대치를 받아 전 세계 GK의 16.6%가 매 시즌 득점했다
# (2005시즌 실측 25,670명: 0골 83.4% / 1골 13.3% / 2골 3.0% / 3골 0.2%).
# 기존 값(ST~CM)은 "이미 밸런싱 끝난 값"이라 전혀 건드리지 않고 GK 항목만
# 새로 추가한다. 팀 강도항(_d*0.2)도 GK만 0으로 두어야 강팀 GK가 확정 득점을
# 받지 않는다 — game_engine._POS_TEAM_STRENGTH_COEF 주석 참고.
#
# 0.48은 실제 코드의 득점 생성식(g_base × _tailed_season_mult + 뎁스 감쇠 +
# 팀 득점 예산 배분)을 그대로 태운 헤드리스 실측(1시즌, GK 약 25,900명)으로
# 고른 값이다 — 이론값이 아니라 측정값:
#     기준치 0.05 → 0골 100%    (꼬리가 아예 사라짐, 과교정)
#     기준치 0.30 → 1골 0.08%   (너무 희귀)
#     기준치 0.45 → 1골 1.05% / 2골 0.10% / 3골 0.004%
#     기준치 0.48 → 1골 1.64% / 2골 0.13% / 3골 0.004%   ← 채택
#     기준치 0.50 → 1골 1.99% / 2골 0.15% / 3골 0.008%
#     기준치 0.55 → 1골 3.05% / 2골 0.25%  (목표 상한 초과)
# 신민용 목표치(0골 97~99% / 1골 1~2% / 2골 0.1~0.3% / 3골+ 극소수) 정중앙.
# 도움도 같은 값을 쓴다(실측 1도움 1.62% / 2도움 0.07%).
...
COUNTRY_OVR_ADJ = {
    ...
    "튀르키예":   +3,
    # [2026-09 확인, 신민용 확정 — 사우디 빅4 재조정] 초기 생성(database.py
    # _generate_team_players)의 실제 OVR 상한은 COUNTRY_LEAGUE_OVR_OVERRIDE
    # (아래 참고, 이번에 68~79로 낮춤)가 이미 대체해버려서 이 값과는
    # 무관해졌지만, ai_lifecycle.py(은퇴자 교체·성장 상한 등 4곳)는 이
    # 값을 COUNTRY_LEAGUE_OVR_OVERRIDE와 별개로 "성장 가능 상한"에 그대로
    # 더 얹어 쓰고 있어(team_cap = override상한 + 대륙보정+국가조정 + 3)
    # 아직 살아있는 값이다 — 빅4 전용 보정은 database.py의
    # GLOBAL_PRESTIGE_STAR_CFG["사우디아라비아"]로 새로 옮겼지만, 이 값
    # 자체는 ai_lifecycle의 저 4곳까지 전부 다시 실측하기 전까지는 그대로
    # 둔다(건드리면 초기 생성과 무관한 별도 시스템까지 검증 없이 흔들리게
    # 됨 — 범위를 좁게 유지).
    "사우디아라비아": +6,
...
# ══════════════════════════════════════════════════════════════
# [2026-09 신설, 신민용 제공 실측표: "COUNTRY_TOP_SCORER_GOALS" —
# 국가별 실제 득점왕 평균 골 수(최근 시즌 실측/추정 기반, FIFA 랭킹이
# 아니라 "리그 득점 환경"을 나타내는 시뮬레이션용 기준값)]
#
# 위 COUNTRY_GOAL_ENV_MULT(16개국만 등록)는 _estimate_ai_season의
# 개인별 "추정치" 단계에만 곱해지는데, 국내 리그 실제 기록(세계기록실에
# 뜨는 값)은 _snapshot_season_ratings에서 이 추정치를 다시
# _apply_team_goal_budget으로 "그 팀이 실제로 넣은 진짜 시즌 골 합계
# (teams.goals_for)"에 맞춰 강제로 재분배한다 — 그런데 이 실제 골
# 합계를 만드는 _gen_score(outcome, diff)는 diff(팀간 OVR차)만 보고
# 국가 정보를 전혀 안 쓴다. 즉 국가별 배율이 "개인 추정치"에만 곱해지고
# 그 추정치는 나중에 팀 실제 총량으로 재조정되면서 지워지므로(같은
# 나라 선수 전원에게 동일 배율을 곱하면 팀 내 상대적 비중은 그대로라
# 재분배 결과에 영향이 없다 — 배율이 한 나라 선수 전체에 균일하게
# 적용되기 때문), 국내 리그 득점왕 실측치엔 사실상 반영되지 않고
# 있었다(신민용 리포트: "득점환경 v1.0 적용 후 대한민국 득점왕이 30골
# 나옴 — 목표는 17골 안팎인데").
#
# 이 표는 그 문제를 고치기 위해 매치 스코어 생성(_gen_score) 자체에
# 국가별 배율을 곱하는 용도다 — get_country_goal_mult() 참고. 기존
# COUNTRY_GOAL_ENV_MULT/get_goal_env_mult는 그대로 유지(국제대회
# 개인추정 등 다른 경로에서 계속 쓰임), 이 표는 완전히 별도 시스템.
COUNTRY_TOP_SCORER_GOALS = {
    "아르헨티나": 20.0,
    "프랑스": 23.0,
    "스페인": 25.0,
    "잉글랜드": 23.0,
...
# "득점환경 v1.0"이 EPL(잉글랜드) 실측으로 튜닝된 앵커이므로, 잉글랜드를
# 배율 1.00 기준점으로 고정하고 나머지 국가는 그 대비 상대 배율로 계산한다.
_GOAL_ENV_ANCHOR_COUNTRY = "잉글랜드"
_GOAL_ENV_ANCHOR_GOALS = COUNTRY_TOP_SCORER_GOALS.get(_GOAL_ENV_ANCHOR_COUNTRY, 23.0)


def get_country_goal_mult(country_name: str) -> float:
    """국가명 -> _gen_score()가 실제로 생성하는 매치 스코어에 곱할 배율.
    COUNTRY_TOP_SCORER_GOALS[country] / 앵커(잉글랜드 23골)로 계산 —
    "이 나라 득점왕 평균이 몇 골이어야 하는가"라는 구체적 실측 기준을
    매치 결과 생성 단계까지 직접 전달한다(위 get_goal_env_mult와 달리
    이건 팀 실제 골 합계 자체를 바꾸므로 _apply_team_goal_budget
    재분배를 거쳐도 지워지지 않는다). 등록 안 된 나라는 안전하게
    1.0(무보정)."""
    if not country_name or country_name not in COUNTRY_TOP_SCORER_GOALS:
        return 1.0
    return COUNTRY_TOP_SCORER_GOALS[country_name] / _GOAL_ENV_ANCHOR_GOALS
...
COUNTRY_LEAGUE_OVR_OVERRIDE = {
    ...
    "카타르": (67, 74),
    # [2026-09 재조정, 신민용 확정: "사우디는 prestige_clubs.py 소속 팀
    # (알힐랄/알나스르/알이티하드/알아흘리)만 84~88, 나머지 1부는 J리그
    # 상/중/하위권보다 낮게, 2부 이하도 하위권에 맞춰서"] 예전 (83,90)은
    # "사우디 전체"에 걸리는 단일 범위라 빅4와 나머지 14개 클럽이 사실상
    # 구분이 안 됐다(실측: 빅4 84~86 vs 비간판 83~85, 차이 1~2점) — 빅4
    # 전용 보정을 database.py의 GLOBAL_PRESTIGE_STAR_CFG["사우디아라비아"]
    # (min_level=2 이상, 즉 빅4 전원 해당)로 완전히 분리했으므로, 여기
    # 범위는 "빅4를 제외한 나머지 1부"의 실제 수준만 표현하면 된다 —
    # 일본(78,86)보다 확실히 낮게 (68,79)로 낮췄다(헤드리스 재현 250회
    # 평균: 비간판 상위권 73.0/중위권 71.2/하위권 69.5 — 일본 최상위
    # 80.7/중위 79.8/최하위 78.3보다 전 구간에서 낮음). get_ovr_range의
    # tier 델타 전파로 2부(59~70)/3부(51~60)도 이 하한을 따라 자동으로
    # 같이 낮아진다(별도 tier별 지정 불필요). 빅4는 이 범위와 무관하게
    # GLOBAL_PRESTIGE_STAR_CFG의 el_base/extra_bonus로 84~88(헤드리스
    # 재현: 알힐랄 84.0~85.9, 나머지 3팀 82.7~84.8)에 별도로 도달한다.
    "사우디아라비아": (68, 79),
    "일본": (78, 86),
    ...
    # [2026-09 재조정, 신민용 확정 — 실제 세이브(11시즌차) 실측 기준 6개국
    # 목표평균(잉글랜드92/스페인91.5/이탈리아91/독일90.5/프랑스90/포르투갈88)
    # 재조정. 게임 내부 생성함수(_target_ovr/_star_target_ovr 등)를 그대로
    # 불러와 실제 팀명·명문가중치로 성숙 이전(생성 시점) 분포를 헤드리스로
    # 재현하고, "현재 override → 실측 성숙평균" 차이(성장기여분, 국가별
    # +7.3~+8.6)를 역산해 새 override가 각 목표평균에 도달하도록 lo/hi를
    # 그리드서치했다(하한/상한 격차 5 미만은 튀르키예식 압축 재발 방지를
    # 위해 제외). 상한도 전 국가 96~97대로 낮춰 실측 최고치(98~99)를
    # 완화한다 — 정밀 시뮬레이션 기반 추정치이므로, 몇 시즌 더 돌려보고
    # 실측이 어긋나면 재조정 필요.
    "포르투갈": (83, 96),
    # [2026-09 확인, 신민용 요청] 네덜란드/벨기에는 포르투갈 새 목표(88)
    # 아래로 이미 정상 위계(87.3/85.7 실측)를 유지하고 있어 이번엔 손대지
    # 않는다.
    "네덜란드": (82, 94),
    "벨기에": (81, 93),
    ...
    "아랍에미리트": (65, 70),
    "중국": (55, 61),
    # [2026-09 버그수정, 신민용 리포트: "튀르키예가 스페인·독일·프랑스보다도
    # 세게 나온다" — 실측(1부 평균91.1·최고99·97+비율13.5%)이 스페인(91.1)과
    # 사실상 동률, 독일(89.8)·프랑스(89.4)보다도 높게 나와 목표 위계("포르투갈>
    # 네덜란드>벨기에>아르헨티나=터키")와 완전히 어긋남을 확인] git 히스토리
    # 대조 결과 직전 값은 (83,89)였는데 09-04 커밋에서 (93,94)로 바뀌어 있었다
    # — 하한93·상한94라는 1점짜리 폭 자체가 이례적이라 실수로 보인다. 하한/
    # 상한 모두 직전 값으로 되돌린다(헤드리스 재현 결과 성숙평균 82~86대로
    # 예상 — 네덜란드·벨기에와 비슷하거나 그 아래, 목표 위계에 부합).
    "튀르키예": (83, 89),
    ...
    # 맞춘다.
    # [2026-09 재조정, 신민용 확정 — 6개국 목표평균 재설정] 실측(잉글랜드91.3/
    # 스페인91.1/이탈리아90.4/독일89.8/프랑스89.4/포르투갈89.7)이 목표(각각
    # 92/91.5/91/90.5/90/88)와 어긋나 재조정.
    # [2026-09 후속 수정, 신민용 지적: "prestige_clubs.py에 속한 5대리그
    # 명문팀은 최대 99까지는 가능해야 한다"] 위 1차 조정에서 상한(hi)까지
    # 96~97로 낮췄더니 명문팀(레벨3, 스타슬롯에 +3 보너스가 tier_top 위에
    # 얹히는 방식)의 99 도달 여지가 사실상 사라졌다 — 잘못된 접근이었다.
    # 상한은 5개국 전부 99로 되돌리고(이탈리아·독일·프랑스도 기존 98→99로
    # 상향 — "이 5개국은 사실상 동급"이라는 요청 반영), 목표평균은 하한
    # (lo)만으로 맞춘다 — 헤드리스 재현 결과 하한을 국가별로 1점 안팎만
    # 올려도 hi=99 자체가 주는 상승분(스타슬롯 목표가 tier_top에 직접
    # 연동)만으로 목표평균에 근접했다(그리드서치, 성장기여분 국가별
    # +7.3~+8.6 역산 적용). tier2~5는 이번에도 손대지 않음.
    "스페인": {1: (91, 99), 2: (82, 90), 3: (74, 84), 4: (64, 74), 5: (53, 63)},
    ...
    "이탈리아": {1: (89, 99), 2: (81, 89), 3: (73, 83), 4: (63, 73), 5: (52, 62)},
    "독일": {1: (88, 99), 2: (81, 89), 3: (73, 83), 4: (63, 73), 5: (52, 62)},
    "프랑스": {1: (87, 99), 2: (82, 90), 3: (74, 84), 4: (64, 74), 5: (53, 63)},
    # [2026-09 재조정, 신민용 확정: "브라질 리그를 포르투갈 정도로 내려서
    # A급 최상으로, OVR도 지금보다 좀 내리고"] 브라질이 COUNTRY_LEAGUE_
    ...
    # [2026-09 후속 수정, 신민용 지적: "브라질도 혹시 몰라서, 최대치 97로
    # 줄여줘"] 실측 확인 결과 실제로 이미 최고98까지 나오고 있었다(포르투갈과
    # 같은 패턴 — override 상한보다 성장분이 몇 점 더 얹힘). 헤드리스
    # 재현으로 "상한을 어디까지 낮춰야 실측 최고가 97 근처로 내려오는지"
    # 역산 — 94→3(93)로 1점만 낮춰도 시뮬 관측 최고가 97→96으로 내려가는
    # 것을 확인, 실측치가 시뮬보다 대체로 1점가량 더 높게 나오는 경향(위
    # 포르투갈 사례와 동일)까지 감안해 상한을 94→93으로 낮춘다(하한은
    # 그대로 — 평균은 이번 요청 범위 밖).
    "브라질": (86, 93),
    # [2026-09 재조정] 위 "6개국 목표평균 재설정" + 후속수정(상한 99 복원)과
    # 같은 작업 — 잉글랜드도 목표평균 92·상한 99(기존과 동일하게 유지)로
    # 맞춘다. 하한만 91→92로 1점 상향. tier2 이하는 손대지 않음.
    "잉글랜드": {1: (92, 99), 2: (85, 92), 3: (76, 86), 4: (64, 74), 5: (53, 63)},
