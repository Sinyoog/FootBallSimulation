...
CONF_CUP_NAME = {
    ...
    "아프리카": "아프리카 네이션스컵",
    # 하위호환
    "남미": "코파 아메리카", "북미": "코파 아메리카", "북중미": "코파 아메리카",
    "오세아니아": "아시안컵",
}
...
OVR_RANGES = {
    ...
    # 밀리는 역전이 있었다. 89~99로 상향 + database.py의 스타슬롯 배정도
    # SS 2부만 예외적으로 후하게 조정(강등팀 낙하산 지원금 반영) — 둘을
    # 합쳐 실측 평균 OVR ≈90.7 (스페인 세군다 ≈80.7 대비 뚜렷하게 위).
    "SS":{1:(90,100),2:(89,99),3:(68,80),4:(53,65),5:(40,52),6:(28,40)},
...
COUNTRY_OVR_ADJ = {
    # ── A급 내부 서열 (실측 순위 기준) ──────────────────────────────
    "포르투갈":   +3,
    "네덜란드":   +2,   # (기존 +3에서 하향 — 실측상 S급보다 낮은 A급 상위일 뿐)
    "벨기에":     +1,
    "아르헨티나": +1,
    "튀르키예":     0,
    "사우디아라비아": +3,  # PIF 오일머니로 슈퍼스타 다수 영입 (품질 자체도 A급 중상위)
    "미국":       +2,   # MLS 인프라+스타 영입(메시 등) 확대
    "멕시코":       0,
    "일본":       -1,   # J리그 — 실측상 A급 하위권(스위스와 비슷한 수준)
    "스위스":     -5,
    "덴마크":     -6,
    "오스트리아": -6,
    "스코틀랜드": -7,
    ...
    # ── B급 최상위 클러스터 (신민용 지정) ───────────────────────────
    # [2026-07 재수정] 앞서 +8은 신민용님이 지적한 "OVR순위 30위"가 아니라
    # "연봉순위 24위"와 혼동해서 나온 값이었다 — 실측 OVR순위(30위)는 이
    # B급 클러스터(크로아티아~우크라이나, 22~28위)보다 아래, 콜롬비아 등
    # (31~33위, adj 없음)보다는 위여야 한다. 실제 생성공식으로 역산해
    # +4로 낮춤(평균 OVR ≈77.7 — 콜롬비아 76.8과 크로아티아 78.6 사이).
    "대한민국":   +4,
    "폴란드":     +3,
    "그리스":     +3,
    "노르웨이":   +3,
