# -*- coding: utf-8 -*-
"""match_sim/sim_engine.py — 경기 시뮬 엔진 (Qt 의존성 0).

## 이름에 대해 — `game_engine.py`와 무엇이 다른가

  `game_engine.py`   : **게임 전체** 엔진. 확률 모델로 경기 "결과"(스코어,
                       team_stats, 개인 기록)를 뽑고, 시즌/이적/커리어까지
                       굴린다. 피치 위 좌표는 다루지 않는다.
  `sim_engine.py`(여기) : **경기 시뮬** 엔진. 그 결과를 입력으로 받아
                       피치 위 22명의 좌표를 매 프레임 굴린다. 스코어를
                       정하지 않는다.

즉 이건 게임 엔진이 아니라 "경기 한 판의 온-피치 시뮬레이터"다. 클래스
이름도 `MatchSimEngine`이며, `game_engine`과 헷갈리지 않도록 항상 이
구분을 지킨다.

## 이 파일이 생긴 이유

원래 이 코드는 전부 `ui/match_sim_viewer.py`의 `MatchSimViewer(QDialog)`
안에 있었다. 시뮬레이션이 UI 클래스 안에 살고 있었다는 뜻이고, 그 결과:

  * UI 없이는 시뮬을 돌릴 수 없어서 **자동 검증이 사실상 불가능**했다.
    (headless_motion_probe.py가 offscreen Qt를 띄우는 우회로를 써야 했고,
    그마저도 시스템에 Qt 런타임이 없으면 아예 못 돌린다.)
  * 그래서 움직임 개선이 매번 "눈으로 보고 → elif 추가"가 됐고, 같은
    증상이 서로 다른 함수에서 반복해서 "근본 원인"으로 지적되며 파일이
    4천 줄을 넘겼다.
  * 시뮬을 game_engine 쪽으로 승격시키려 해도 QDialog에 묶여 있어 못 한다.

이 파일은 그 이음매를 끊는다. 여기엔 Qt import가 한 줄도 없고, 따라서:

    from match_sim.sim_engine import MatchSimEngine
    eng = MatchSimEngine(data)
    eng.simulate()          # 프레임 굽기 (경기당 약 0.25초)
    eng.frames              # 궤적 로그

만으로 헤드리스 배치 검증이 가능하다. `ui/match_sim_viewer.py`는 이제
이 로그를 **그리기만 하는** 얇은 렌더러다.

## 경계

  엔진(여기)   : 22명 상태, 대형/역할 계산, 볼/패스, 데드볼 상태머신,
                 씬 연출, 프레임 굽기. 순수 파이썬.
  렌더러(저기) : QDialog/QPainter, 재생 타이머, 시크바, 프레임 보간.

`frames`의 각 원소는 `_snapshot_frame()`이 만드는 dict이며 이게 두 층
사이의 **유일한 계약**이다. 렌더러는 엔진의 내부 상태를 읽지 않는다
(포지션 라벨과 팀 이름 같은 정적 메타데이터만 예외).

## 알려진 구조적 문제 (motion_metrics.py로 계량됨)

이 파일은 **기존 동작을 그대로 옮긴 것**이고 움직임 자체는 아직 고치지
않았다. 베이스라인 계측 결과 확인된 것:

  * `_advance_one_tick`은 경기 시계를 `_FRAME_DT`(0.06분 = 3.6초)씩
    진행시키는데, `_update_player_positions`는 물리를 `dt = 0.12`(초)로
    적분한다 — **30배 시간 불일치**. 선수는 3.6초가 흐르는 동안 0.12초
    분량만 움직인다. 그래서 선수당 이동거리가 실축의 35%(3.2km),
    정지 시간 비율 74%가 나온다.
  * 목표 좌표가 전부 `pl["hx"] + adv` 형태라, 선수는 포메이션 기준점에
    스프링으로 묶여 있다.
  * 역할이 단일 인덱스(_presser_idx / _support_idx / ...)로만 할당되어,
    한 프레임에 유의미하게 움직이는 선수가 평균 1.4명이다(실축 8~18).

이 세 가지가 다음 단계(공간 모델 / 공 물리 / 의사결정 루프)의 대상이다.
"""
...
# [Phase 2] 공간 모델 — 목표 좌표를 포메이션 상수표가 아니라 피치 가치
# 필드의 argmax로 정한다. 세 모듈 다 Qt/엔진에 의존하지 않는 순수 계산이라
# 단독 테스트가 가능하다.
from match_sim import roles as _roles
from match_sim import space_model as _space
from match_sim import positioning as _positioning
...
def _smooth_damp(current, target, velocity, smooth_time, dt, max_speed):
    ...
    orig_to = target
    # [버그 수정 — Phase 2] 예전엔 `max_speed * smooth_time` 이었다.
    # SmoothDamp에서 이 값은 사실상 "이번 스텝의 최대 변위"로 작동하는데,
    # smooth_time(0.30초)이 실제 틱 길이(3.6초)보다 훨씬 짧아서 한 틱에
    # 최대 max_speed×0.3 = 약 2.4m 밖에 못 움직였다. 틱이 아무리 길어져도
    # 마찬가지라, dt를 30배 늘려봐도 이동량이 포화됐다(실측 확인).
    # 물리적으로 맞는 상한은 "이 시간 동안 최고속도로 갈 수 있는 거리"다.
    max_change = max_speed * max(smooth_time, dt)
...
# [Phase 2] 목표까지의 반응 시간(초). 예전 _SMOOTH_TIME(0.30)은 틱 길이가
# 0.12초라는 잘못된 전제 위에서 맞춘 값이었다. 실제 틱은 3.6초이므로,
# "방향을 바꿔 목표로 향하는 데 걸리는 시간"으로서 현실적인 값을 쓴다.
_SMOOTH_TIME_S = 0.55


# [Phase 2] 가속 한계(m/s²). 실축에서 선수의 최대 가속도는 대략 3~4 m/s²
# 수준이다(정지→최고속도까지 2~3초). 이 값이 없으면 선수가 한 틱 만에
# 0 → 8 m/s로 튀었다가 목표 도착 후 다시 0이 되는 "순간이동-정지" 패턴이
# 나온다(실측: 스프린트 175회/경기, 정지 시간 65%).
_MAX_ACCEL_MS2 = 3.6
# 목표에 다가갈 때 감속을 시작하는 여유 계수. 클수록 부드럽게 선다.
_ARRIVE_SLOW_S = 0.9


def _steer_toward_ms(pl, tx, ty, dt, vmax_ms, smooth_time=None):
    """[Phase 2] 가속도 제한이 있는 운동학 적분기.

    [왜 SmoothDamp를 버렸나] 기존 `_steer_toward`는 SmoothDamp(임계감쇠
    스프링)를 썼는데, 그 안의 변위 상한이 `max_speed * smooth_time`이라
    **틱 길이(dt)와 무관**했다. dt가 smooth_time보다 커지는 순간 감쇠항이
    사실상 사라져서, 선수가 한 틱에 목표까지 최고속도로 직행한 뒤 멈춘다.
    이게 "정지해 있다가 순간이동한다"는 인상의 물리적 원인이었다.

    여기서는 그냥 실제로 하는 대로 한다 — 원하는 속도를 정하고, 가속도
    한계 안에서 현재 속도를 그쪽으로 바꾸고, 그 속도로 움직인다. 도착
    근처에서는 원하는 속도를 줄여서 오버슈트 없이 선다.

    좌표계 주의: x는 피치 길이(105m), y는 폭(68m) 축이라 정규화 단위의
    실제 크기가 다르다. 전부 미터로 바꿔서 계산한 뒤 되돌린다.
    """
    LX, LY = _space.PITCH_LEN_M, _space.PITCH_WID_M
    px, py = pl["x"] * LX, pl["y"] * LY
    vx, vy = pl["vx"] * LX, pl["vy"] * LY
    dx, dy = tx * LX - px, ty * LY - py
    dist = math.hypot(dx, dy)

    if dist < 1e-6:
        desired_vx = desired_vy = 0.0
    else:
        # [긴급도] 실축에서 선수는 대부분의 시간을 걷거나 조깅으로 보내고,
        # 전력질주는 경기당 20~40회뿐이다. 목표가 가까우면(자리 미세조정)
        # 천천히, 멀면(뒷공간 침투/수비 복귀) 전력으로 간다. 이게 없으면
        # 목표가 조금만 떨어져 있어도 매번 최고속도로 달려서 스프린트가
        # 경기당 170회씩 찍힌다(실측).
        urgency = min(1.0, max(0.31, dist / 22.0))
        # 도착 감속 — 남은 거리를 _ARRIVE_SLOW_S 안에 소진할 속도로 제한
        speed_cap = min(vmax_ms * urgency, dist / max(1e-3, _ARRIVE_SLOW_S))
        desired_vx = dx / dist * speed_cap
        desired_vy = dy / dist * speed_cap

    # 가속도 한계
    ax, ay = desired_vx - vx, desired_vy - vy
    amag = math.hypot(ax, ay)
    amax = _MAX_ACCEL_MS2 * dt
    ...
    pl["x"] = min(0.97, max(0.03, px / LX))
    pl["y"] = min(0.95, max(0.05, py / LY))
    pl["vx"] = vx / LX
    pl["vy"] = vy / LY
...
class MatchSimEngine:
    """경기 한 판을 시뮬레이션해 궤적 로그(frames)를 만든다.

    Qt에 의존하지 않으므로 헤드리스 배치/회귀 검증에 그대로 쓸 수 있다.
    """

    TICK_MS = 20
    # [Phase 2] 0.06분(3.6초) → 0.02분(1.2초). 예전 값은 한 프레임 사이에
    # 선수가 목표까지 다 가버리고 남은 시간을 서 있는 구조를 강제했다
    # (계측: 정지 시간 90%, 최고속도 26m/s — 즉 "순간이동 후 대기").
    # 실제 움직임을 담으려면 틱이 이동 시간보다 짧아야 한다.
    # 씬/배너 등 "틱 수" 기반 연출은 프레임 수가 같으므로 그대로 유지되고,
    # 그것들이 소비하는 경기 시간만 짧아진다.
    _FRAME_DT = 0.02
    _SEC_PER_MIN = 4.0
    ...
    def __init__(self, data):
        ...
        self._TRANSITION_DURATION = 3.0   # 이 시간(초) 동안 부스트가 감쇠
        # [Phase 2] 0.55 → 0.18. 예전엔 속도가 정규화 단위였고 상한이
        # 따로 없어서 +55%가 눈에 띄는 연출이었지만, 이제 vmax가 실제
        # m/s라 +55%는 최고속도를 14 m/s까지 밀어올린다(인간 한계 밖).
        self._TRANSITION_BOOST = 0.18     # 최대 부스트 시 속도 +18%
    ...
    def simulate(self):
        """프레임을 이미 __init__에서 굽는 현행 동작을 명시적으로 노출한다.
        (호출자가 `eng.simulate()`라고 쓸 수 있게 하는 얇은 진입점 — 나중에
        굽기를 __init__ 밖으로 빼낼 때 호출부를 안 고치기 위한 자리.)"""
        return self.frames
    ...
    def _reset_live_state(self):
        ...
        self.ball_trail = []
        self._halftime_reset_done = False
        # [Phase 2] 역할 가중치 테이블 — 경기 시작 시 1회 계산(스탯 기반
        # 듀티 배분 포함). 리셋 시 비워서 다시 만들게 한다.
        self._role_table = None
        for _t in (self.home_players, self.away_players):
            for _p in _t:
                _p["_tgt_cell"] = None
    ...
    def _export_debug_capture(self):
        ...
        try:
            ...
            # [구조 변경] 예전엔 여기서 바로 QMessageBox를 띄웠다 — 그
            # 한 줄 때문에 이 메서드 전체가 Qt에 묶여서 헤드리스로 못 돌았다.
            # 이제 엔진은 "어디에 저장했는지"만 돌려주고, 사용자에게 알리는
            # 건 호출자(뷰어)가 한다.
    ...
    def _update_player_positions(self, speed_scale=1.0):
        """[Phase 2 재설계] 22명 전원의 목표 좌표를 공간 가치 필드에서 뽑는다.

        ## 무엇이 바뀌었나

        예전 이 함수는 22개의 `if/elif` 가지였고, 각 가지는 미리 지정된
        인덱스 한 명(`_presser_idx`, `_advancing_mid_idx`, `_run_behind_idx`,
        `_volpiana_idx` …)에게만 적용됐다. 어디에도 안 걸린 선수는
        `pl["hx"] + adv` 라는 사실상 정적인 목표를 받았다. 계측 결과가
        그대로였다 — 한 프레임에 유의미하게 움직이는 선수 1.4명(실축 8~18),
        정지 시간 74%.

        이제 전원이 같은 규칙을 돈다: `match_sim/positioning.py`가 피치
        격자의 공간 가치 필드를 만들고, 각 선수는 자기 역할 가중치
        (`match_sim/roles.py`)로 그 필드를 본 뒤 argmax 칸을 목표로 잡는다.
        서로 다르게 움직이는 이유는 분기가 달라서가 아니라 가중치와 존이
        달라서다.

        ## 여기 남아있는 것 (일부러 남긴 것들)

          * **씬/코너/GK 락 스킵** — 다른 코드가 좌표를 직접 통제하는
            선수는 건드리면 안 된다. 예전과 동일.
          * **홀더 / 압박수비 / GK** — 이 셋은 공간 필드가 아니라 각자
            고유한 규칙을 쓴다. 홀더는 드리블 목표, 압박수비는 볼 자체,
            GK는 스위퍼 라인. (압박수비 인덱스는 `_do_pass_or_turnover`의
            턴오버 판정도 쓰므로 `_assign_roles`는 그대로 유지된다.)
          * **최소 수비 깊이 / GK 간격 / 오프사이드 클램프** — 전부 실측
            버그에서 나온 안전장치라 그대로 둔다.
          * **shape_push / _lagged_ball_*** — 씬 코드가 참조하므로 계속
            갱신한다(포지셔닝에는 더 이상 쓰이지 않는다).

        ## dt 수정

        예전엔 `dt = 0.12`(초)로 하드코딩돼 있었는데, `_advance_one_tick`은
        경기 시계를 `_FRAME_DT`(0.06분 = 3.6초)씩 밀고 있었다 — **30배
        불일치**. 선수는 3.6초가 흐르는 동안 0.12초 분량만 움직였다.
        이제 경기 시계에서 직접 뽑는다.
        """
        dt = self._FRAME_DT * 60.0 * max(1.0, speed_scale)

        # ── shape_push / 지연 볼 / 전환 부스트 ──
        # 포지셔닝은 더 이상 이 값들을 쓰지 않지만, 씬(_start_scene_body,
        # _advance_scene)과 GK 스위퍼 라인이 참조하므로 계속 갱신한다.
        for side in ("home", "away"):
            if side == self.possession:
                target = 1.0
            else:
                own_goal_x = 0.0 if side == "home" else 1.0
                target = 0.12 + 0.56 * abs(self.ball["x"] - own_goal_x)
        ...
        # ── 좌표를 직접 통제당하는 선수 집합(여기서 건드리면 안 됨) ──
        skip = {"home": set(), "away": set()}
        ...
        if scene_active:
            ...
            _atk_side = self._scene_side
            _def_side = "away" if _atk_side == "home" else "home"
            skip[_atk_side] |= atk_skip
            skip[_def_side] |= def_skip
        ...
        if not corner_lock_active and (self._corner_lock_home or self._corner_lock_away):
            self._corner_lock_home = set()
            self._corner_lock_away = set()
        if corner_lock_active:
            skip["home"] |= set(self._corner_lock_home)
            skip["away"] |= set(self._corner_lock_away)
        ...
        if (self._db is not None and self._db.get("kind") == "shot"
            ...
            skip[shot_gk_lock[0]].add(shot_gk_lock[1])

        # ── 역할 테이블(경기 시작 시 1회) ──
        if getattr(self, "_role_table", None) is None:
            self._role_table = {
                "home": _roles.build_role_table(self.home_players)[0],
                "away": _roles.build_role_table(self.away_players)[0],
            }

        # ── 최고속도(m/s) 주입 — 도달시간/점유 계산이 이걸 쓴다 ──
        for side, team in (("home", self.home_players), ("away", self.away_players)):
            rt = self._role_table[side]
            for i, pl in enumerate(team):
                _spd = (pl.get("stats") or {}).get("speed", 50)
                # 인간 상한(약 9.5 m/s)을 넘지 않게 최종 클램프 — 스탯·
                # 전환부스트·반응성이 곱해지면 쉽게 넘어간다.
                pl["vmax_ms"] = min(9.4, rt[i]["vmax"] * (0.80 + _spd / 100.0 * 0.40)
                                    * transition_mult * pl.get("reaction", 1.0) ** 0.5)

        # ── 공간 가치 필드에서 22명 목표 좌표 ──
        targets, field = _positioning.compute_targets(
            self.home_players, self.away_players,
            self.ball["x"], self.ball["y"], self.possession,
            self._role_table["home"], self._role_table["away"],
            skip_home=skip["home"], skip_away=skip["away"])

        grid = _space.default_grid()

        for side, team in (("home", self.home_players), ("away", self.away_players)):
            sign = 1 if side == "home" else -1
            push = self.shape_push[side]
            is_holder_side = (side == holder_side)
            atk_goal_x = 1.0 if side == "home" else 0.0
            _gk_idx_this_side = next(
                (k for k, _p in enumerate(team) if _p["pos"] == "GK"), 0)
            tgt_list = targets[side]
            ...
            for i, pl in enumerate(team):
                ...
                if pl["pos"] == "GK":
                    # GK 스위퍼 라인. 공간 필드에 맡기면 골문을 비우므로
                    # 전용 규칙을 쓴다 — 다만 예전처럼 골문 앞에 얼어붙게
                    # 두지는 않는다(실측: GK 이동거리 0.8km / 실축 4~6km,
                    # 정지 시간 95%).
                    #
                    #  x: 백라인이 올라갈수록 같이 나와 뒷공간을 커버하고,
                    #     볼이 우리 진영 깊이 들어오면 골문으로 붙는다.
                    #  y: 슛 각도를 좁히려면 볼과 골대 중앙을 잇는 선 위에
                    #     서야 한다 — 예전 계수 0.05는 사실상 안 움직인
                    #     것이라 0.34로 올렸다(골문 폭을 벗어나지 않도록
                    #     아래에서 클램프).
                    _own_goal = 0.0 if sign > 0 else 1.0
                    _ball_far = abs(self.ball["x"] - _own_goal)
                    _sweep = 0.03 + push * 0.09 + max(0.0, _ball_far - 0.45) * 0.16
                    target_x = _own_goal + _sweep * sign
                    # [데드밴드] 볼 y를 매 틱 그대로 따라가게 했더니 GK가
                    # 좌우로 끊임없이 흔들려 이동거리가 18km까지 치솟았다
                    # (실축 4~6km). 볼이 가까울 때만, 그리고 목표가 일정
                    # 폭 이상 벗어났을 때만 자리를 옮긴다.
                    _near = max(0.0, min(1.0, (0.55 - _ball_far) / 0.55))
                    _raw_ty = 0.5 + (self.ball["y"] - 0.5) * (0.10 + 0.24 * _near)
                    _raw_ty = min(0.66, max(0.34, _raw_ty))
                    _prev_ty = pl.get("_gk_ty")
                    if _prev_ty is None or abs(_raw_ty - _prev_ty) > 0.045:
                        pl["_gk_ty"] = _raw_ty
                        _prev_ty = _raw_ty
                    target_y = _prev_ty
                elif is_holder_side and i == self.holder:
                    # 홀더는 존을 무시하고 자기 주변에서 가장 좋은 칸으로
                    # 몰고 간다(드리블 전개). 공은 아래 _update_possession이
                    # 홀더 발밑에 얹어서 따라가므로 순환참조가 없다.
                    target_x, target_y = _positioning.dribble_target(
                        pl, field, grid, atk_goal_x)
                elif (not is_holder_side) and i == presser_idx:
                    # 압박수비 — 볼 자체로 다가간다. 이 선수의 실제 거리가
                    # _do_pass_or_turnover의 인터셉트 판정에 쓰이므로,
                    # 공간 필드가 아니라 볼을 직접 쫓아야 한다.
                    target_x = pl["x"] + (self.ball["x"] - pl["x"]) * 0.75
                    target_y = pl["y"] + (self.ball["y"] - pl["y"]) * 0.75
                else:
                    t = tgt_list[i]
                    if t is None:
                        continue
                ...
                if pl["pos"] != "GK":
                    # 오프사이드 라인 — 상대 최종 수비보다 터무니없이 앞서
                    # 정적으로 서 있는 장면을 막는다(간이 판정).
                    if is_holder_side:
                        _opp = self.away_players if side == "home" else self.home_players
                        _xs = [p["x"] for p in _opp if p["pos"] != "GK"]
                        if _xs:
                            if sign > 0:
                                target_x = min(target_x, max(_xs) + 0.04)
                            else:
                                target_x = max(target_x, min(_xs) - 0.04)
                    # 최소 수비 깊이 — 아무리 로우블록이어도 골라인까지
                    # 밀려나지는 않는다.
                    ...
                    # GK보다는 항상 앞에 — GK 스위퍼가 나와 있을 때 CB가
                    # 골키퍼보다 골대에 가까워지는 실측 버그 방지.
                    _gk_x = team[_gk_idx_this_side]["x"]
                    if sign > 0:
                        target_x = max(target_x, _gk_x + 0.06)
                    else:
                        target_x = min(target_x, _gk_x - 0.06)

                target_x = min(0.97, max(0.03, target_x))
                target_y = min(0.95, max(0.05, target_y))

                reaction = pl.get("reaction", 1.0)
                _steer_toward_ms(pl, target_x, target_y, dt,
                                 pl.get("vmax_ms", 7.0),
                                 _SMOOTH_TIME_S / reaction)

        # ── 같은 팀 겹침 방지 ──
        # 씬/코너 크라우드처럼 의도적으로 밀집해야 하는 선수는 제외하고,
        # 일반 이동 중인 같은 팀 선수끼리만 최소 간격을 확보한다.
        _MIN_TEAMMATE_DIST = 0.02
        for side, team in (("home", self.home_players), ("away", self.away_players)):
            sk = skip[side]
            free_idx = [i for i, pl in enumerate(team)
                        if pl["pos"] != "GK" and i not in sk]
            for a in range(len(free_idx)):
                ...
                for b in range(a + 1, len(free_idx)):
                    ...
                    if 0.0001 < dist < _MIN_TEAMMATE_DIST:
                        pushv = (_MIN_TEAMMATE_DIST - dist) * 0.5
                        nx, ny = dx / dist, dy / dist
                        pa["x"] = min(0.97, max(0.03, pa["x"] + nx * pushv))
                        pa["y"] = min(0.95, max(0.05, pa["y"] + ny * pushv))
                        pb["x"] = min(0.97, max(0.03, pb["x"] - nx * pushv))
                        pb["y"] = min(0.95, max(0.05, pb["y"] - ny * pushv))
                    elif dist <= 0.0001:
                        pushv = _MIN_TEAMMATE_DIST * 0.5
                        pa["x"] = min(0.97, max(0.03, pa["x"] + pushv))
                        pb["x"] = min(0.97, max(0.03, pb["x"] - pushv))
    ...
    def _update_possession(self, speed_scale=1.0):
        ...
        if self._pass_flight:
            ...
            if self.pass_clock <= 0:
                ...
                # [Phase 2] dt가 실제 초 단위가 됐으므로 실축 값으로.
                # 한 선수가 볼을 갖고 있는 시간은 대략 1.5~4초다.
                self.pass_clock = random.uniform(1.5, 4.0)
